...
Generated by `custom_access_token_hook` function (auth hook):
...
2. Queries user's properties and communities
3. Injects claims into JWT
4. RLS policies read from `auth.jwt()` function
5. No database queries needed for authorization checks
