...
> **See Also:** [auth-and-session.md](./auth-and-session.md) for complete application-level auth implementation details
...
#### Address resolution (source precedence)

To locate an address, check the sources in order and stop at the first hit:

1. **`public.custom_address`** — if found, use it.
2. **`public.project_addresspoints`** — if found, use it.
3. **Geoscape exact match** — `process_geoscape_geocoder_response(address_string text)`:
   - `matchType: "exact"` → look up `propid` via the returned `cadastralIdentifier`.
   - `matchType: "neighbour"` → treat as a validated address; look up `propid` via `cadastralIdentifier`.
   - `matchType: "noMatch"` → the address cannot be located.
4. **NSW SS `principaladdresssiteoid`** — `get_principaladdresssiteoid_from_cadastralidentifier(cadastral_identifier text)`; a valid integer proceeds, otherwise the lookup has failed.
5. **NSW SS `AddressPoint`** — query the SSNSW `PropertyAddress` MapServer for the point geometry
   and address strings, e.g.:

   ```sql
   SELECT * FROM http_get('https://maps.six.nsw.gov.au/arcgis/rest/services/sixmaps/PropertyAddress/MapServer/2/query?where='
     || urlencode('AddressPoint.principaladdresssiteoid=4570028')
...
This decision tree checks multiple sources to find the most accurate location.
...
**For auth implementation:** See [auth-and-session.md](./auth-and-session.md)
