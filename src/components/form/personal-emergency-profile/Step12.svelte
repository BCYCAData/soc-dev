<script lang="ts">
	import TextAreaInput from '$components/form/inputs/TextAreaInput.svelte';
	import type { TinoneeCommunityProfile } from '$lib/form.types';

	interface Props {
		communityTinoneeProfile: TinoneeCommunityProfile;
		communityTinoneeInformationOptions?: { value: string; lable: string }[];
	}

	let { communityTinoneeProfile, communityTinoneeInformationOptions = [] }: Props = $props();

	let formState = $derived({
		information_sheet_choices: communityTinoneeProfile?.information_sheet_choices ?? [],
		other_information_sheet: communityTinoneeProfile?.other_information_sheet ?? ''
	});

	$effect(() => {
		if (communityTinoneeProfile) {
			communityTinoneeProfile.information_sheet_choices = formState.information_sheet_choices;
			communityTinoneeProfile.other_information_sheet = formState.other_information_sheet;
		}
	});
</script>

<h2 class="h2 text-surface-950-50 mb-1 text-lg font-semibold">
	What Tinonee Community information would be useful to you?<span
		class="text-scale-3 text-surface-500 ml-2"
	>
		(Check all that apply)</span
	>
</h2>
{#if communityTinoneeProfile}
	<div
		class="bg-secondary-200-800 grid grid-flow-col gap-2 rounded-lg p-2 sm:grid-cols-2 sm:grid-rows-4 sm:gap-2"
	>
		{#each communityTinoneeInformationOptions as { value, lable } (value)}
			<div class="col-span-1 flex items-center">
				<input
					class="ml-8 h-6 w-6"
					name="information_sheet_choices"
					type="checkbox"
					bind:group={formState.information_sheet_choices}
					value={Number(value)}
					checked={formState.information_sheet_choices.includes(Number(value))}
				/>
				<label
					class="text-scale-6 text-secondary-900-100 ml-2 font-medium"
					for="information_sheet_choices">{lable}</label
				>
			</div>
		{/each}
	</div>

	<TextAreaInput
		headingClass="h2 mb-1 text-lg font-semibold text-surface-950-50"
		headingText="Is there other information which you would find useful?"
		lableClass={null}
		lableText={null}
		divClass="p-2 rounded-lg bg-secondary-300-700 sm:text-scale-5"
		nameText="other_information_sheet"
		textAreaClass="w-full resize-y sm:text-scale-5"
		bind:inputValue={formState.other_information_sheet}
	/>
{/if}
