...
export async function generatePropertyMap(property: PropertyData): Promise<Buffer> {
	...
	// Draw boundary using GeoJSON coordinates
	if (property.boundary.geometry.type === 'Polygon') {
		const coordinates = property.boundary.geometry.coordinates[0];
		coordinates.forEach((coord, index) => {
			const [x, y] = projectToCanvas(coord, width, height);
			if (index === 0) {
				ctx.moveTo(x, y);
			} else {
				ctx.lineTo(x, y);
	...
	property.assets.forEach((asset) => {
		const [x, y] = projectToCanvas([asset.lng, asset.lat], width, height);
		ctx.fillStyle = asset.type === 'water_tank' ? 'blue' : 'green';
		ctx.beginPath();
...
function projectToCanvas(coord: number[], width: number, height: number): [number, number] {
	// Simple linear projection - you might want to use a proper map projection library
	const [lng, lat] = coord;
