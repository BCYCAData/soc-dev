...
import { getLogo } from '$lib/server/pdf/config/pdfConfig';
...
export async function generateRfsStreetReport({
	...
	const documentDefinition: TDocumentDefinitions = {
		footer: (currentPage: number, pageCount: number) => ({
			text: [
				{ text: `Generated by: ${generatedBy} | `, style: 'footer' },
...
function buildReportContent(streetName: string, propertyData: RfsReportData[]): Content[] {
	...
	content.push(
		...
		{ text: streetName, style: 'title_h1' }
	);

	// Property sections with new components
		...
		stack: [
			{
				table: {
					widths: ['*'] as string[],
					body: [
						[{ text: property.address, style: 'tableHeader', alignment: 'center' as Alignment }],
						[{ text: `Landline: ${property.phone}`, alignment: 'center' as Alignment }]
					]
				},
				layout: 'noBorders'
			}
...
function buildFireAssetsTable(property: RfsReportData) {
	return {
		stack: [
			{ text: 'Fire Fighting Assets', style: 'sectionHeader' },
			{
				table: {
					widths: ['auto', '*'],
					body: [
...
function buildResidentsTable(property: RfsReportData): Content {
	const residentContent: ContentStack = {
		stack: [
			{ text: 'Residents', style: 'sectionHeader' },
