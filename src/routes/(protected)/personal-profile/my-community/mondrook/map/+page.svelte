<script lang="ts">
	...
	const mondrookAddressPoints = $derived(data.addressPoints);
	const mondrookRegisteredPoints = $derived(data.registeredPoints);
	const mondrookCommunityArea = $derived(data.community);
