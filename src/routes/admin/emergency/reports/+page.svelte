<script lang="ts">
	import '$lib/pdf/Poppins-Regular-normal.js';
	import '$lib/pdf/Poppins-SemiBold-normal.js';
	import { pdf, setFrontPage } from '$lib/pdf/helper';
	...
	$: ({ streetList } = data);
	$: reportStreet = form?.reportStreet;
	$: streetData = form?.streetData;
	$: residentData = form?.residentData;
...
<pdf.tabWrapper>
	<pdf.tabHead>
		...
		<pdf.tabHeadItem id={'1'} on:click={handleTabClick('1')} {activeTabValue}
			>RFS Street Report</pdf.tabHeadItem
		>
		<!-- svelte-ignore a11y-click-events-have-key-events -->
		<pdf.tabHeadItem id={'2'} on:click={handleTabClick('2')} {activeTabValue}
			>RFS Property Report</pdf.tabHeadItem
		>
	</pdf.tabHead>
	<pdf.tabContentItem id={'1'} contentDivClass="p-4 bg-primary-300 rounded-b-lg" {activeTabValue}>
		<form
		...
		>
			<div class="flex flex-col basis-full mx-5">
				...
				<button
					on:click={() => printPDF(streetData, residentData)}
					class="w-1/4 m-3 rounded-lg text-base font-semibold bg-[#0099E8] text-stone-100 border border-purple-700"
					form=""
					hidden={streetData === undefined}
				>
		...
		</form></pdf.tabContentItem
	>
	<pdf.tabContentItem id={'2'} contentDivClass="p-4 bg-primary-300 rounded-b-lg" {activeTabValue}
