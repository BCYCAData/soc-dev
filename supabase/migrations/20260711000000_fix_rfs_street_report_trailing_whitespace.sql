-- RFS Street Report: streets with trailing whitespace in the stored address
-- could never be reported on. get_street_list() trims entries for the
-- dropdown, but get_rfs_property_data_for_street() matched the RAW column with
-- a suffix LIKE, so rows like '636 Kimbriki Rd ' never matched their own
-- dropdown entry and the report endpoint returned 404 ("File wasn't available
-- on site" in the browser).
--
-- Fix in two parts:
-- 1. Make the report's match whitespace-safe on both sides. The inclusive
--    suffix-LIKE semantics are kept deliberately: an entry like
--    'HILLVILLE ROAD' must keep matching '"EUROKA", 77 HILLVILLE ROAD' —
--    dropping prefixed variants would silently omit properties from an
--    emergency report.
-- 2. One-off trim of legacy address data (idempotent).
--
...
-- One-off cleanup of legacy addresses stored with leading/trailing whitespace
-- (the source of the unmatchable dropdown entries). The rtrim() in the
-- function above keeps the report safe if dirty data ever reappears.
UPDATE public.property_profile
SET property_address_street = trim(property_address_street)
WHERE property_address_street <> trim(property_address_street);
